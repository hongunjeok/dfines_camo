...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.last_sem_feats = None  # 학습 시 두 스케일 feature를 잠시 보관
    ...
    def forward(self, feats):
        ...
        # --- store semantic features for loss (train only) ---
        if self.training:
            self.last_sem_feats = [
                proj_feats[2],  # global (stride=32)
                proj_feats[1],  # mid (stride=16)
                proj_feats[0],  # local (stride=8)
            ]
        else:
            self.last_sem_feats = None
